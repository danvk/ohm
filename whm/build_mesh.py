    ...
    *different* WHM country lies within `land_border_radius` degrees of it.
    ...
    for f in admin0_features:
        ...
        for lon, lat in feature_verts(f):
            ...
            if any(
                n[2] != name and dist2((lon, lat), (n[0], n[1])) < land_border_radius**2
                for n in neighbours
            ):
...
def main():
    ...
    print(
        f"  delta_lon: mean={sum(dlons) / len(dlons):+.3f}°  "
        f"rms={math.sqrt(sum(d**2 for d in dlons) / len(dlons)):.3f}°"
    )
    print(
        f"  delta_lat: mean={sum(dlats) / len(dlats):+.3f}°  "
    ...
    if args.plot:
        try:
            ...
            for ax, arr, title, clim in zip(
                ...
                im = ax.pcolormesh(L, B, arr, cmap="RdBu_r", vmin=-clim, vmax=clim)
